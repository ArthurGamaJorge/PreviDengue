...
export default function Detectar() {
  ...
  const displayNome = (key: string) =>
    nomes_por_classe[key] ?? key.replace(/_/g, " ");
  ...
  const maxIntensityRaw = Math.max(
    0,
    ...images.map((im) => Number(im.result?.intensity_score ?? 0))
  );
  const getScaledIntensity = (raw: number) =>
    maxIntensityRaw > 0 ? Math.min(10, (raw / maxIntensityRaw) * 10) : 0;

  // Habilita export apenas quando há pelo menos uma imagem processada com intensidade
    ...
    () =>
      images.filter(
        (img) =>
          !img.loading &&
          img.result &&
          typeof img.result.intensity_score === "number"
      ).length,
    [images]
  );
  ...
  const handleExportJson = () => {
    if (!canExport) return; // guarda de segurança
    ...
    const items = processed.map((img) => {
      const raw = Number(img.result!.intensity_score);
      const intensity = getScaledIntensity(raw);
      return { name: img.name, intensity_raw: raw, intensity };
    });
    ...
    a.href = url;
    a.download = "detect_intensities.json";
    a.click();
  ...
  const onMouseMove = (idx: number, e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setPanOffsets((po) =>
      po.map((v, i) => (i === idx ? { x: v.x + dx, y: v.y + dy } : v))
    );
    setPanStart({ x: e.clientX, y: e.clientY });
  };
  ...
  return (
    <div
      ref={topRef}
      className="min-h-screen font-sans bg-gradient-to-br from-zinc-950 to-zinc-900 text-white px-4 sm:px-8 pb-0 pt-0"
    >
      <Header />
      ...
      <main className="pt-24 flex mx-auto gap-8">
        ...
        <aside className="w-[350px] flex-shrink-0 sticky top-24 self-start space-y-4">
          ...
          {/* Grupo: Filtros de Classes (controles movidos para a direita) */}
          <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 border border-zinc-700 rounded-xl p-6 shadow-lg space-y-6">
            <h3 className="text-zinc-300 font-semibold">Filtrar classes</h3>
              ...
              images.map((img, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    className={`relative w-full group overflow-hidden rounded-xl bg-black/20 select-none overscroll-none touch-none ${
                      isPanning[index] ? "cursor-grabbing" : "cursor-grab"
                    }`}
                  ...
                  >
                    <div className="w-full h-full flex items-center justify-center">
                      <div
                        ...
                        style={{
                          transform: `translate(${
                            panOffsets[index]?.x ?? 0
                          }px, ${panOffsets[index]?.y ?? 0}px) scale(${
                            zoomScales[index] ?? 1
                          })`,
                          transformOrigin: "0 0",
                          transition: "none",
                        ...
                        <div className="absolute inset-0 pointer-events-none select-none">
                          {img.result?.objetos
                            ...
                            .map((det: any, i: number) => {
                              const box = det.box;
                              const confidence = (det.confidence * 100).toFixed(
                                1
                              );
                              const label = `${displayNome(
                                det.class
                              )} (${confidence}%)`;
                              ...
                              const leftPercent =
                                (box.x1 / originalWidth) * 100;
                              const topPercent =
                                (box.y1 / originalHeight) * 100;
                              const widthPercent =
                              ...
                              <div className="flex flex-wrap gap-2 mt-4">
                                {Object.entries(contagemFiltrada).map(
                                  ([classe, quantidade], i) => (
                                    <div
                                      key={i}
                                      className="flex items-center gap-2 bg-zinc-800 border border-zinc-700 px-3 py-1 rounded-full text-zinc-200"
                                    >
                                      <span
                                        className="w-2 h-2 rounded-full"
                                        style={{
                                          backgroundColor:
                                            cores_por_classe[classe],
                                        }}
                                      ></span>
                                      <span className="font-semibold">
                                        {displayNome(classe)}: {quantidade}
                                      </span>
                                    </div>
                                  )
                                )}
        ...
        {rightOpen && (
          <aside className="w-[320px] hidden lg:block flex-shrink-0 sticky top-24 self-start">
            <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 border border-zinc-700 rounded-xl p-6 shadow-lg">
              {/* Controles movidos para a direita */}
              <div className="space-y-4 mb-4">
                <div>
                  ...
                  >
                    ...
                    <Slider.Thumb className="block w-4 h-4 bg-white rounded-full shadow-md hover:bg-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors" />
                  </Slider.Root>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-zinc-400 text-xs hover:text-white border border-zinc-600 hover:border-white px-4 py-2 rounded-lg transition-colors text-sm"
                  >
                    Limpar imagens
                  </button>
                </div>
              ...
              <button
                ...
                className={`text-xs px-2 py-2 rounded border w-full mb-4 ${
                  !canExport
                ...
                title="Exportar JSON (nome, intensidade total e normalizada; maior risco primeiro)"
              >
                Exportar JSON
              </button>
              {images.length === 0 ? (
                ...
                <ul className="space-y-3 max-h-[60vh] overflow-auto pr-1">
                  {(() => {
                    ...
                    const ordered = sortByRisk
                      ? [...items].sort((a, b) => b.scaled - a.scaled)
                      : items;
